...
class GraphGC extends JFrame implements ActionListener, ComponentListener {
  ...
  public GraphGC(GCSample gcSample) {
    ...
    Color compileColor = JBColor.CYAN;//Color.getColor("graphgc.compile.color", Color.WHITE);
  ...
  private void updateGraph(GCSample gcSample) {
    ...
    if (!this.inGC) {
      ...
      if (this.inEdGC || this.inTnGC || this.inStopGC) {
        this.inGC = true;
        if (this.inEdGC) {
...
//        System.out.println("stopGCTimeDelta:" + stopGCTimeDelta);
  ...
  private void updateTextComponents(GCSample gcSample) {
    ...
    title = MessageFormat.format(Res.getString("gc.collectors.n.time"),
        GCSample.collector0name, Converter.longToTimeString(lastEdenGCTimeDelta, GCSample.osFrequency),
        gcSample.edenGCEvents, Converter.longToTimeString(gcSample.edenGCTime, GCSample.osFrequency));
    //+ ", Max:" + Converter.longToTimeString(edenGcTimeDataSet.getMaxValue(), GCSample.osFrequency);
    titledBorder.setTitle(title);

    titledBorder = (TitledBorder) this.tenuredGCTimePanel.getBorder();
    title = MessageFormat.format(Res.getString("gc.collectors.n.time"),
        GCSample.collector1name, Converter.longToTimeString(lastOldGCTimeDelta, GCSample.osFrequency),
        gcSample.tenuredGCEvents, Converter.longToTimeString(gcSample.tenuredGCTime, GCSample.osFrequency));
    titledBorder.setTitle(title);

    if (GCSample.collector2name != null) {
      titledBorder = (TitledBorder) this.stopGCTimePane.getBorder();
      title = MessageFormat.format(Res.getString("gc.collectors.n.time"),
          GCSample.collector2name, Converter.longToTimeString(laststopGCTimeDelta, GCSample.osFrequency),
          gcSample.stopGCEvents, Converter.longToTimeString(gcSample.stopGCTime, GCSample.osFrequency));
      titledBorder.setTitle(title);
    }
